...
\title[BreedingMarts]{Marts - a data integration technology with applications in plant information management}
\author[Sample \textit{et~al}]{Reinhard Simon\,$^{1,*}$, Raul Cordova\,$^{2}$ and Co-Author\,$^2$\footnote{to whom correspondence should be addressed}}
